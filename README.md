...
Uygulama Modülleri:
Tespit Modu (tani.py): Doğrudan CMD (Komut Satırı) üzerinden çalışır. Statik bir Windows penceresi açarak modelin anlık tanıma yapmasını sağlar.
Kamera Modu (kamera.py): Bilgisayarın kamerasından anlık canlı görüntü alarak ekranda atıkları tespit eder.
